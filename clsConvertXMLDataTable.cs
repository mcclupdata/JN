...
namespace JN_WELD_Service
{
    static class clsConvertXMLDataTable
    {
        ...
        /// <returns></returns>
        static public DataTable ConvertXMLToDataTableWithChange(string xmlData)
        {
            StringReader stream = null;
            XmlTextReader reader = null;
            try
            {
                ...
                reader = new XmlTextReader(stream);//获取stream中的数据
                xmlDS.ReadXml(reader, XmlReadMode.DiffGram);//DataSet获取Xmlrdr中的数据
        ...
        /// <returns></returns>
        static public DataTable ConvertXMLtoDataSet(String xmlData, DataSet xmlDS)
        {
            StringReader stream = null;
            XmlTextReader reader = null;
            try
        ...
        static public DataTable ConvertXMLToDataTable(string xmlData)
        {
            ...
            try
            {
                ...
                stream = new StringReader(xmlData);//读取字符串中的信息
                reader = new XmlTextReader(stream);//获取stream中的数据
                if (xmlData.IndexOf("<diffgr:diffgram") <0 )
                {
